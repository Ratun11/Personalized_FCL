...
def fedavg(thetas: List[qml.numpy.tensor]):
    w = qml.numpy.zeros_like(thetas[0])
    for th in thetas:
        w = w + th / len(thetas)
    return qml.numpy.array(w, requires_grad=True)

def local_train(theta_global, obj_fn, local_steps: int, lr: float):
    theta = qml.numpy.array(theta_global, requires_grad=True)
    opt = qml.AdagradOptimizer(stepsize=lr)
    for _ in range(local_steps):
        theta, _ = opt.step_and_cost(obj_fn, theta)
    return qml.numpy.array(theta, requires_grad=True)

def run_one(device: str, shots: Optional[int], seed: int, n_qubits: int, layers: int,
            K: int, rounds: int, local_steps: int, lr: float, gamma: float,
            phi: np.ndarray, objective: str):
    dev = make_device(device, wires=n_qubits, shots=shots)
    qnode = build_qnode(dev, n_qubits=n_qubits, layers=layers)

    theta_g = init_theta(seed, n_qubits, layers)
    ...
    rows = []
    t0 = time.time()
    rows.append({"round": 0, "global_cost": float(obj(theta_g)), "avg_client_cost": np.nan})
    for r in range(1, rounds+1):
        client_thetas = []
        client_costs = []
        for _ in range(K):
            th_k = local_train(theta_g, obj, local_steps=local_steps, lr=lr)
            client_thetas.append(th_k)
            client_costs.append(float(obj(th_k)))
        theta_g = fedavg(client_thetas)
        rows.append({"round": r, "global_cost": float(obj(theta_g)), "avg_client_cost": float(np.mean(client_costs))})
...
def main():
    ...
    p.add_argument("--out_csv", default="fcl_iid_results.csv")
    args = p.parse_args()
    ...
    all_runs = []
    run_id = 0
    for cfg in config_product(shots=shots_list, n_qubits=qubits_list, layers=layers_list, K=K_list, gamma=gamma_list):
        run_id += 1
        n = int(cfg["n_qubits"])
        ...
        df["run_id"] = run_id
        all_runs.append(df)
        print(f"[FCL_IID] run {run_id}: obj={df['objective'].iloc[0]} shots={cfg['shots']} qubits={n} layers={cfg['layers']} K={cfg['K']} gamma={cfg['gamma']} final_global={df['global_cost'].iloc[-1]:.6f}")

    out = pd.concat(all_runs, ignore_index=True)
    save_csv(args.out_csv, out)
    print(f"[FCL_IID] saved: {args.out_csv} (rows={len(out)})")
